...
function sayGoodbye(myName) {
  var firstString = "Goodbye, ";
  var middleString = myName;
  var lastString = ". Have a great day.";

  var wholeString = firstString + middleString + lastString;
...
*/
function temperatureInF(value, type) {
  var returnValue = "";
  if(type == 'F'){
    returnValue = value.toString() + type;
  } 

  if(type == 'C'){
    returnValue =  temperatureCtoF(value).toString() + 'F';
  }
...
function getName(myObject) {
  ...
  var secondString = myObject.name.toString();
...
function getModelYears(inventory) {
  var newArray = new Array(inventory.length);

  for(var i = 0; i < inventory.length; i++) {
    var yearVar = inventory[i].car_year;
    newArray[i] = yearVar;
  }
